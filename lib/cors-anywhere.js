...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // Extract valid X-Requested-With from the query string
    if (req.url.indexOf('x-requested-with=') !== -1) {
      var match = req.url.match(/(^|&|\?)x-requested-with=([^&]+)(&|$)/);
      if (match) {
        req.headers['x-requested-with'] = decodeURIComponent(match[2]);
        req.url = req.url.replace(match[0], match[1] === '?' ? '?' : match[3] ? match[1] : '');
        // Clean up creating valid url if we removed the only query param
        if (req.url.slice(-1) === '?') {
    ...
    var urlWithoutSlash = req.url.slice(1);
    
    // Auto-fix: Correct URLs like /https:/hostname/path to /https://hostname/path
    // This handles cases where reverse proxy removes one slash
    if (/^https?:\/[^/]/i.test(urlWithoutSlash)) {
      // Check if it's followed by a hostname (not just a single character)
      var fixedUrl = urlWithoutSlash.replace(/^(https?:\/)([^/])/i, '$1/$2');
      console.log('[URL Auto-Fix] Corrected missing slash:', {
        original: req.url,
        corrected: '/' + fixedUrl,
        urlWithoutSlash: urlWithoutSlash,
        fixedUrl: fixedUrl
      });
      urlWithoutSlash = fixedUrl;
      req.url = '/' + fixedUrl; // Update req.url for logging consistency
    }
    
    // Check if URL appears to be missing protocol (starts with hostname-like pattern)
    if (!/^https?:\/\//i.test(urlWithoutSlash) && /^[a-z0-9.-]+\.[a-z]{2,}/i.test(urlWithoutSlash)) {
      res.writeHead(400, 'Missing protocol', cors_headers);
      res.end('The URL must include a protocol (http:// or https://). Example: /https://' + urlWithoutSlash + ' instead of /' + urlWithoutSlash);
      return;
    }

    var location = parseURL(urlWithoutSlash);
    ...
    if (!location) {
      ...
      if (/^\/https?:\/[^/]/i.test(req.url)) {
        ...
        res.end('The URL is invalid: two slashes are needed after the http(s):. Received URL: "' + req.url + '"');
        return;
      }
      // Detect cases like /proxy.local/https://google.com or /hostname/http://example.com
      // where a hostname is followed by a full URL
      if (/^\/[^\/]+?\/(https?:\/\/)/i.test(req.url)) {
        res.writeHead(400, 'Invalid URL format', cors_headers);
        res.end('The URL is invalid: do not include a hostname before the target URL. Use /https://example.com instead of /hostname/https://example.com');
        return;
      }
    ...
    // Validate that protocol is present and valid
    if (!location.protocol || (location.protocol !== 'http:' && location.protocol !== 'https:')) {
      res.writeHead(400, 'Missing protocol', cors_headers);
      res.end('The URL must include a protocol (http:// or https://). Example: /https://example.com');
      return;
    }
...
exports.createServer = function createServer(options) {
  ...
  proxy.on('error', function(err, req, res) {
    ...
    var isDNSError = err.code === 'EAI_AGAIN' || 
                     err.code === 'ENOTFOUND' || 
                     err.code === 'EAI_NODATA' ||
                     err.code === 'ETIMEDOUT' ||
                     (err.message && /getaddrinfo|DNS|hostname/i.test(err.message));

    var statusCode = isDNSError ? 502 : 404;
    var statusMessage = isDNSError ? 'DNS Resolution Failed' : 'Not Found';
    var errorMessage = isDNSError 
      ? 'DNS resolution failed: Unable to resolve hostname. The hostname may be invalid, unreachable, or there may be a network issue. Error: ' + err.message
      : 'Not found because of proxy error: ' + err;
